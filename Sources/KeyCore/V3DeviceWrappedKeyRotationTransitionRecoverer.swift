...
struct V3DeviceWrappedKeyRotationRecoveryResult: Equatable, Sendable {
    let outcome: V3ImmutableTransactionRecoveryOutcome
    let trustedCheckpoint: V3DeviceWrappedTrustedCheckpoint?
    let vaultKey: Data?
}

struct V3DeviceWrappedKeyRotationRecoveryContext: Sendable {
...
enum V3DeviceWrappedKeyRotationRecoveryValidationError: Error {
    case authenticationCancelled
    case localWrapperInvalid
}

/// Reconstructs one locally anchored device-roster key rotation.
///
/// The exact device-local anchor authorizes only the immutable candidate that
/// passed its fresh enrollment or revocation policy before publication began.
/// Recovery reopens the old and new owner wrappers, authenticates both key
/// epochs, and proves a complete plaintext-preserving reseal before resuming.
struct V3DeviceWrappedKeyRotationTransitionRecoverer: Sendable {
    ...
    func recover(
        ...
        unwrapReason: String,
        validateIntent: (V3ImmutableTransactionRecoveryIntent) -> Bool,
        validate: (
            V3DeviceWrappedKeyRotationRecoveryContext
        ) throws -> V3DeviceWrappedValidatedKeyRotation,
        afterCheckpointAdvance:
            V3DeviceWrappedKeyRotationCommitHandler = { _, _ in }
    ) throws -> V3DeviceWrappedKeyRotationRecoveryResult {
        guard isValidV3UUID(vaultID),
              kind == .enrollDevice || kind == .revokeDevice,
    ...
    private func recover(
        ...
        do {
            ...
            throw V3DeviceWrappedKeyRotationRecoveryValidationError
                .authenticationCancelled
        } catch {
            throw invalidRecoveryState(intent)
        }
        ...
        let validated: V3DeviceWrappedValidatedKeyRotation
        do {
            validated = try validate(
                V3DeviceWrappedKeyRotationRecoveryContext(
                    intent: intent,
                    parent: parent,
        ...
        if currentCheckpoint == candidateCheckpoint {
            ...
            return V3DeviceWrappedKeyRotationRecoveryResult(
