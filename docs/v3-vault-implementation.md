...
| `V2-801` | Planned; next-minor target | Announce v2 retirement with actionable warnings that preserve secret stdout, machine-readable output, and warning-stage exit codes. Qualify explicit v3 setup for new vaults. Existing v2 selections remain usable; no install-time migration or unqualified default switch. |
