...
| `V2-801` | Warning and internal new-vault installer implemented locally; setup command pending | Terminal v2 status and config listing warn on stderr without changing secret stdout, JSON, redirected output, or exit codes. A separate empty-v3 genesis path requires a newly created directory and never accesses a v2 key. Helper-owned selection, interrupted-attempt handling, and CLI qualification remain pending. Existing v2 selections remain usable; no install-time migration or unqualified default switch. |
