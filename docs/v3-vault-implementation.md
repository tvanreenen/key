...
| `V2-801` | Warning implemented locally; new-vault setup pending | Terminal v2 status and config listing warn on stderr without changing secret stdout, JSON, redirected output, or exit codes. Qualify explicit v3 setup for new vaults separately. Existing v2 selections remain usable; no install-time migration or unqualified default switch. |
