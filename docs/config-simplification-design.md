...
First-time enrollment remains a release blocker for this slice. The old joining-Mac flow used config setters to establish a temporary v2 selection before the verified enrollment ceremony. Removing that bootstrap means a Mac with no config cannot yet enter that flow. It needs an explicit existing-folder selection path that installs config only after verified enrollment; init must never fill this role. The remaining UX decision is an explicit vault-directory option versus using the joining CLI's current directory. Already configured enrollment paths remain unchanged. Do not publish this branch as a complete first-time multi-Mac setup story until that entry point is implemented and tested.
...
Review `CFG-801`, the warning slice, explicit new-v3-vault initialization, and removal of implicit setup together as the focused PR. Complete the unconfigured enrollment entry point, then qualify signed-product first-time setup, helper restart, and provider interruption behavior before release. Define the bounded legacy-source record needed for `CFG-802` separately. Keep v2 content-write enforcement, normal-runtime removal, automated failed-init recovery, and catastrophe-recovery format changes in separate reviewed slices. No automatic format conversion, installed-app change, live-config rewrite, provider change, or YubiKey operation is included.
