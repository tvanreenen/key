...
`key init` initializes the current empty directory. `key init <directory>` resolves relative paths against the CLI's working directory and creates only the final directory if missing. Its parent must exist. `--` permits a path beginning with a dash; there is no `--force`, no separate config-selection step, and no existing-vault reinitialization. CLI warnings identify this as new-vault creation, direct other-Mac users to enrollment, and disclose the current permanent-loss boundary before requesting key creation. Secret stdout and other commands are unchanged.

`KeyServiceHost` owns the unconfigured-to-configured transition under an exclusive request barrier. It composes the ordinary `KeyServiceHandler` lazily and preserves its existing request/mutation ownership once selected. This keeps initialization out of v2 runtime composition; adding a special case to the old startup path would first bootstrap the very legacy config that init must avoid. Unconfigured utility status and lock requests do not create configuration, and human/JSON vault-status requests report that setup is needed. Existing configurations still use the selected runtime; ordinary legacy bootstrap remains available for compatibility. Init refuses any existing config object or already composed runtime instead of switching it.
...
Review `CFG-801`, the warning slice, and explicit new-v3-vault initialization together as the focused PR. Qualify signed-product first-time setup, helper restart, and provider interruption behavior before release. Define the bounded legacy-source record needed for `CFG-802` separately. Keep write enforcement, normal-runtime removal, automated failed-init recovery, and catastrophe-recovery format changes in separate reviewed slices. No implicit default-format switch, v2 write restriction, installed-app change, live-config rewrite, provider change, or YubiKey operation is included.
